...
def intersetPoint(head1,head2):
    ...
    if(n>m):
        while(head1):
            head1.data=[sys.maxsize,head1.data]
            head1=head1.next
        while(head2.next):
            try:
                if(head2.next.data[0]==sys.maxsize):
                    return head2.next.data[1]
            except:
                head2=head2.next
        return -1
    else:
        while(head2):
            head2.data=[sys.maxsize,head2.data]
            head2=head2.next
        while(head1.next):
            try:
                if(head1.next.data[0]==sys.maxsize):
                    return head1.next.data[1]
            except:
                head1=head1.next
        return -1
